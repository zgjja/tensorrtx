...
int main(int argc, char** argv) {
    ...
    const std::string img_path = "../assets/kids.png";
    auto firstProb = doInference(*context, img_path, N);
    printFirstOutputs("alexnet", firstProb[0].data(), firstProb[0].size());
    ...
    for (int32_t i = 0; i < kBenchmarkRuns; ++i) {
        ...
        (void)doInference(*context, img_path, N);
    ...
    printBenchmark("alexnet", latencies, N);
