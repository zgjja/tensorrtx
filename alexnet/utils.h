...
static ILayer* addTransformLayer(INetworkDefinition* network, ITensor& input, bool bgr2rgb,
    ...
    const std::array<const float, 3> shift_ = {-mean[0] / std[0], -mean[1] / std[1], -mean[2] / std[2]};
    const std::array<const float, 3> scale_ = {1 / (std[0] * 255), 1 / (std[1] * 255), 1 / (std[2] * 255)};
    const Weights empty{DataType::kFLOAT, nullptr, 0ll};
    const Weights shift{DataType::kFLOAT, &shift_, 3ll};
    const Weights scale{DataType::kFLOAT, &scale_, 3ll};

    // Convert input image from Uint8 to FP32
    auto* identity = network->addIdentity(input);
    assert(identity);
    identity->setName("Convert to FP32");
#if TRT_VERSION < 11000
