...
static auto make_layers(INetworkDefinition* net, WeightMap& w, ITensor& input, const LayerConfig& cfg,
                        const std::string& name = "frontend.", bool use_bn = false, bool use_dilation = false)
        -> ILayer* {
    auto* tensor = &input;
    int32_t idx = 0;
    ILayer* ret = nullptr;
...
auto createEngine(int32_t maxBatchSize, IRuntime* runtime, IBuilder* builder, IBuilderConfig* config, DataType dt)
        -> ICudaEngine* {
