...
static ILayer* addTransformLayer(INetworkDefinition* network, ITensor& input, bool bgr2rgb,
    ...
    static const std::array<const float, 3> shift_ = {-mean[0] / std[0], -mean[1] / std[1], -mean[2] / std[2]};
    static const std::array<const float, 3> scale_ = {1 / (std[0] * 255), 1 / (std[1] * 255), 1 / (std[2] * 255)};
