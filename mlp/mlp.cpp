...
ICudaEngine* createMLPEngine(int32_t N, IRuntime* runtime, IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    // clang-format off
    // Add layer for MLP
    auto* fc1w_tensor = network->addConstant(Dims4{1, 1, 1, 1}, weightMap["linear.weight"])->getOutput(0);
    auto fc1b_tensor = network->addConstant(Dims4{1, 1, 1, 1}, weightMap["linear.bias"])->getOutput(0);
    assert(fc1w_tensor && fc1b_tensor);
    auto* fc1w = network->addMatrixMultiply(*data, MatrixOperation::kNONE, *fc1w_tensor, MatrixOperation::kTRANSPOSE);
    auto fc1b = network->addElementWise(*fc1w->getOutput(0), *fc1b_tensor, ElementWiseOperation::kSUM);
    assert(fc1w && fc1b);
    fc1w->setName("fc1w");
    fc1b->setName("fc1b");
    // clang-format on
...
int main(int argc, char** argv) {
    ...
    // deserialize engine from the char-stream
#if TRT_VERSION >= 8000
